...
function showUndo(){let p;try{p=JSON.parse(sessionStorage.getItem(UNDO_KEY)||'null')}catch{}if(!p||Date.now()-p.at>12000){sessionStorage.removeItem(UNDO_KEY);return}let el=$('#uxUndo');if(!el){el=document.createElement('div');el.id='uxUndo';el.className='ux-undo';document.body.appendChild(el)}const confirmable=String(p.label||'').startsWith('已批量')||p.label==='滚动项目已删除'||p.label==='已暂停滚动';el.innerHTML=`<span>${esc(p.label||'已更新')}</span><button data-ux="undo">撤销</button>${confirmable?'<button data-ux="confirm-undo">确定</button>':''}`;el.classList.add('show');clearTimeout(showUndo.t);showUndo.t=setTimeout(()=>{el.classList.remove('show');sessionStorage.removeItem(UNDO_KEY)},9000)}
function undo(){let p;try{p=JSON.parse(sessionStorage.getItem(UNDO_KEY)||'null')}catch{}if(!p?.state)return;write(p.state);sessionStorage.removeItem(UNDO_KEY);if(['滚动项目已删除','已暂停滚动'].includes(p.label))sessionStorage.setItem('ux-restore-view','rolling');location.reload()}
function completeInitial(taskId){const state=read(),task=(state?.tasks||[]).find(t=>t.id===taskId&&t.type==='rolling'&&t.rollingInitial);if(!state||!task)return;task.done=true;syncLinkedTasks(state);write(state);sessionStorage.setItem('ux-restore-view','rolling');location.reload()}
function mutateOne(action,id,index){const state=read(),item=findItem(state,id),node=findNode(item,index);if(!state||!item||!node)return;if(action==='complete'){const raw=prompt('本次复习用时（分钟，可留空）','');if(raw===null){sessionStorage.removeItem(UNDO_KEY);return}const minutes=Math.max(0,Number(raw)||0);node.status='done';node.completedAt=new Date().toISOString();node.minutes=minutes;rollingRecord(state,item,node,minutes);markLinkedTask(state,item,node,true)}else if(action==='delay'){node.uxDelayedFrom=today();node.date=add(node.date,1);state.tasks=(state.tasks||[]).filter(t=>!(t.type==='rolling'&&t.rollingId===id&&Number(t.rollingIndex)===Number(index)&&!t.done))}else{node.status='skipped';node.completedAt=new Date().toISOString();markLinkedTask(state,item,node,true)}write(state);sessionStorage.setItem('ux-restore-view','rolling');location.reload()}
function mutateSelected(kind){
  ...
  for(const p of picks){
    if(p.kind==='initial'){const task=(state.tasks||[]).find(t=>t.id===p.taskId),item=findItem(state,p.id);if(!task||!item)continue;if(kind==='complete')task.done=true;else if(kind==='delay'){task.uxDelayedFrom=today();task.date=add(task.date,1)}else pauseIds.add(item.id);continue}
    const item=findItem(state,p.id),node=findNode(item,p.index);if(!item||!node)continue;if(kind==='complete'){node.status='done';node.completedAt=new Date().toISOString();node.minutes=0;rollingRecord(state,item,node,0);markLinkedTask(state,item,node,true)}else if(kind==='delay'){node.uxDelayedFrom=today();node.date=add(node.date,1);state.tasks=(state.tasks||[]).filter(t=>!(t.type==='rolling'&&t.rollingId===p.id&&Number(t.rollingIndex)===p.index&&!t.done))}else pauseIds.add(item.id);
  }
  ...
  syncLinkedTasks(state);write(state);sessionStorage.setItem('ux-restore-view','rolling');location.reload();
}
...
document.addEventListener('click',e=>{
  ...
  if(ux){e.preventDefault();
    ...
    if(ux==='complete-initial'||ux==='early-complete-initial')return completeInitial(uxEl.dataset.taskId);
    if(ux==='complete-one'||ux==='early-complete-one')return mutateOne('complete',uxEl.dataset.id,uxEl.dataset.index);if(ux==='delay-one')return mutateOne('delay',uxEl.dataset.id,uxEl.dataset.index);if(ux==='skip-one')return mutateOne('skip',uxEl.dataset.id,uxEl.dataset.index);
