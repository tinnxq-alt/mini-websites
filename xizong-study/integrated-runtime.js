...
function undo(){let p;try{p=JSON.parse(sessionStorage.getItem(UNDO_KEY)||'null')}catch{}if(!p?.state)return;write(p.state);sessionStorage.removeItem(UNDO_KEY);if(['滚动项目已删除','已暂停滚动','已恢复滚动'].includes(p.label))sessionStorage.setItem('ux-restore-view','rolling');location.reload()}
