...
function completeInitial(taskId){const state=read(),task=(state?.tasks||[]).find(t=>t.id===taskId&&t.type==='rolling'&&t.rollingInitial);if(!state||!task)return;stashUndo('当天滚动学习已完成');task.done=true;syncLinkedTasks(state);write(state);location.reload()}
...
function mutateSelected(kind){
  const picks=selectedDue();if(!picks.length)return toast('先选择要处理的滚动项目');const state=read();if(!state)return;stashUndo(kind==='complete'?'已批量完成':kind==='delay'?'已批量顺延':'已批量暂停');const pauseIds=new Set();
