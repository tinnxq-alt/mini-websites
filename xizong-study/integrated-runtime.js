...
function undo(){let p;try{p=JSON.parse(sessionStorage.getItem(UNDO_KEY)||'null')}catch{}if(!p?.state)return;sessionStorage.removeItem(UNDO_KEY);commitState(p.state)}
...
function earlyCompleteInitial(taskId){const state=read(),task=(state?.tasks||[]).find(t=>t.id===taskId&&t.type==='rolling'&&t.rollingInitial),item=task?findItem(state,task.rollingId):null;if(!state||!task||!item)return;task.uxEarlyCompletedFrom=task.date;task.date=today();task.done=true;syncLinkedTasks(state);commitState(state)}
...
function earlyCompleteOne(id,index){const state=read(),item=findItem(state,id),node=findNode(item,index);if(!state||!item||!node)return;const raw=prompt('本次复习用时（分钟，可留空）','');if(raw===null)return;const minutes=Math.max(0,Number(raw)||0);node.status='done';node.completedAt=new Date().toISOString();node.minutes=minutes;rollingRecord(state,item,node,minutes);ensureEarlyCompletedTask(state,item,node);commitState(state)}
...
function deleteRollingProject(id){const state=read(),item=findItem(state,id);if(!item)return;stashUndo('滚动项目已删除');state.rollingReviews=(state.rollingReviews||[]).filter(r=>r.id!==id);state.tasks=(state.tasks||[]).filter(t=>t.rollingId!==id||t.done);commitState(state)}
function contextualDeleteTask(id){const state=read(),task=(state?.tasks||[]).find(t=>t.id===id);if(!task)return;if(task.type==='rolling')return toast('滚动任务请在“滚动复习”页完成、顺延或暂停');if(!confirm(`删除“${task.name}”？\n日期：${task.date}${task.done?'\n该任务已完成。':''}`))return;stashUndo('任务已删除');if(state.activeTimer?.taskId===id)state.activeTimer=null;if(task.auto){state.customizedDates=state.customizedDates||{};state.customizedDates[task.date]=true}state.tasks=state.tasks.filter(t=>t.id!==id);commitState(state)}
